...
function silowrite{T1<:AbstractString, T2<:Real}(file::silofile, name::T1, x_values::Vector{T2}, y_values::Vector{T2}; optlist::Ptr{DBoptlist}=Ptr{DBoptlist}(0))
  const N = length(x_values)
